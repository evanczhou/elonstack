...
Find the single constraint that sets your production rate. Everything else is subordinate. You can have 9,999 things working perfectly and one broken thing sets the pace. Fix that one thing. Don't optimize anything else until you do.
...
Design is never the bottleneck. Production always is. If the user says their bottleneck is "the product isn't good enough," push back: name the specific production, distribution, or delivery constraint.
